...
- **Prerrequisito:** re-verificar en `metatrader5.com/en/releasenotes` cuál es la build vigente en el momento real de instalar (no asumir que sigue siendo la 6090 — ver [`PREGUNTAS_ABIERTAS.md`](PREGUNTAS_ABIERTAS.md) Q-001).
...
- Verificación tras instalar: comprobar el número de build desde `Ayuda → Acerca de` (o menú equivalente) y registrarlo en [`ESTADO_INVESTIGACION.md`](ESTADO_INVESTIGACION.md).
...
- **Antes de introducir cualquier dato personal:** verificar manualmente en `cnmv.es` el registro/passporting del broker elegido (resuelve [`PREGUNTAS_ABIERTAS.md`](PREGUNTAS_ABIERTAS.md) Q-005).
...
- Definir el mecanismo de sincronización entre el repositorio Git (`wiki-Traiding/proyecto-mt5-bots/`, y el futuro `src/`/`strategies/`) y `MQL5/Experts` — no desarrollar directamente dentro del Data Folder de una instalación real sin control de versiones paralelo.
...
- Registrar en [`ESTADO_INVESTIGACION.md`](ESTADO_INVESTIGACION.md): build exacta instalada, ruta del Data Folder, servidor del broker demo.
...
- Resolver en la práctica [`PREGUNTAS_ABIERTAS.md`](PREGUNTAS_ABIERTAS.md) Q-004: localizar en la UI real del Terminal/MetaEditor la configuración de proveedor de IA, permisos (AI-initiated trading, confirmación manual, red, línea de comandos) y el mecanismo de conexión de un cliente MCP externo.
