...
2. **Consultar documentación oficial ante cualquier duda**, priorizando la jerarquía de fuentes de [`FUENTES.md`](FUENTES.md) (Nivel 1 > Nivel 2 > Nivel 3).
...
20. **Logging suficiente y anti-spam (Límite por barra)** — toda decisión relevante (señal generada, orden enviada, fill recibido, kill switch activado o error real) debe registrarse con `Print()`. Para señales filtradas o no cumplidas, el registro debe limitarse estrictamente a **como máximo una vez por barra** (dentro del guard de nueva barra `iTime`), prohibiéndose llamadas a `Print()` en ramas `else` evaluadas en cada tick para evitar generar archivos de log de múltiples gigabytes que congelen MetaTrader durante backtests masivos.
21. **Código versionado** — ningún EA que vaya a probarse en backtest serio, y mucho menos en demo, existe fuera de un commit identificable (`16_Observabilidad...` §5). Directiva `#property version "x.yy"` obligatoria.
22. **Backtest reproducible** — mismo código + mismo `.set` + mismo dataset/broker + mismo tick model debe producir el mismo resultado; cualquier fuente de no-determinismo (p. ej. uso de `MathRand()` sin semilla fija en la lógica de la estrategia) debe evitarse o documentarse explícitamente. Los supuestos a verificar manualmente deben entregarse junto al código.
...
Rol: desarrollador senior de MQL5 siguiendo el estándar del proyecto
  (wiki-Traiding/proyecto-mt5-bots/17_Estandar_Desarrollo_EAs_con_IA.md).

Contexto obligatorio a proporcionar:
- StrategyProposal completa (wiki-Traiding/proyecto-mt5-bots/11_Arquitectura_Multiagente_Futura.md §5)
- RiskPolicy vigente (wiki-Traiding/proyecto-mt5-bots/05_Gestion_Riesgo_EAs.md §5)
- Plantilla de arquitectura de EA (wiki-Traiding/proyecto-mt5-bots/03_Arquitectura_Expert_Advisors.md §6)
...
2. Arquitectura: generar un archivo .mq5 autocontenido (usando Trade/Trade.mqh de la librería estándar oficial), organizado internamente en secciones modulares claramente delimitadas: Inputs/Parámetros, Hard Limits/RiskManager, Position Sizer, Generador de Señal, Ejecución CTrade y OnTradeTransaction.
3. Cumplir TODAS las 22 reglas de la sección 1 de este documento sin excepción.
