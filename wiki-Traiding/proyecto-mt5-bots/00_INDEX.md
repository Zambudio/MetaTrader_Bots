...
### Documentos de seguimiento y meta

No forman parte de la tabla anterior (no son temáticos ni llevan estado `BORRADOR`/`VERIFICADO`), pero completan el corpus de los 26 documentos contados en "Resultado" más abajo:

- [`ESTADO_INVESTIGACION.md`](ESTADO_INVESTIGACION.md) — estado de avance de la investigación, Definition of Done y snapshot de archivos creados por sesión.
- [`FUENTES.md`](FUENTES.md) — catálogo de fuentes oficiales revisadas (F001-F030), con nivel de fiabilidad y fecha de consulta.
- [`PREGUNTAS_ABIERTAS.md`](PREGUNTAS_ABIERTAS.md) — preguntas Q-001 a Q-005 pendientes de verificación práctica, ninguna bloqueante para iniciar la Fase 1.
- [`DECISIONS/README.md`](DECISIONS/README.md) — formato y flujo de los Architecture Decision Records (ADR) de este proyecto; índice vacío por ahora (ningún ADR creado todavía).
- [`raw/README.md`](raw/README.md) — convención del `raw/` propio de este corpus (fuentes inmutables citadas en `FUENTES.md`).
- [`raw/Web_METATRADER5.md`](raw/Web_METATRADER5.md) — fuente inmutable (F004 en [`FUENTES.md`](FUENTES.md)): informe elaborado el 06/08/2026 a partir del sitio web oficial de MetaTrader 5, usado como punto de partida de esta investigación pero **no** como fuente de verdad (contrastado y corregido, ver [`00_Contexto_y_Objetivos_Proyecto.md`](00_Contexto_y_Objetivos_Proyecto.md) §7 y `01_Arquitectura_y_Funcionamiento_MetaTrader5.md` §1).
...
Ver [`PREGUNTAS_ABIERTAS.md`](PREGUNTAS_ABIERTAS.md) (Q-001 a Q-005). Ninguna es bloqueante para **iniciar** la Fase 1, pero Q-004 y Q-005 deben resolverse antes de completar los Pasos 2 y 6 de `19_Plan_Puesta_en_Marcha.md` respectivamente.
...
- **Dato corregido respecto al prompt maestro:** la build "más reciente" no es la 6060 sino la 6090; documentado en `01_...` §1 y en [`ESTADO_INVESTIGACION.md`](ESTADO_INVESTIGACION.md).
...
- **Enlaces internos:** todos los documentos referenciados desde este índice existen en `wiki-Traiding/proyecto-mt5-bots/` (verificado por listado de archivos; corpus movido aquí desde `docs/MetaTrader/` el 2026-08-19, ver `wiki-Traiding/log.md`).
...
Ver "Definition of Done" en [`ESTADO_INVESTIGACION.md`](ESTADO_INVESTIGACION.md) y el resumen final que se añadirá tras la auditoría cruzada. En resumen: todos los documentos marcados como críticos (`Bloquea puesta en marcha = Sí`) deben estar en estado `VERIFICADO` o `REVISADO` sin preguntas bloqueantes abiertas.

---

_Este índice se actualiza tras cada bloque completado. Ver [`ESTADO_INVESTIGACION.md`](ESTADO_INVESTIGACION.md) para el detalle de progreso._
