...
## [2026-08-30] audit | Generación MQL5 resiliente a reinicios/túnel + backtest que aborta si MT5 ya está abierto

> Nota: las tres sesiones del 2026-08-29 (`selector-fuente-modelo-cli`, `loop-quality-gate-verde-eurusd`, `verificacion-fix-condicion-entrada-y-afinado-prompts`) quedaron indexadas en [`proyecto-dashboard/00_INDEX.md`](proyecto-dashboard/00_INDEX.md) pero sin entrada en este log — hueco conocido, pendiente de backfill si se quiere el registro cronológico completo.

Pedro reportó, sobre la web ya publicada en `trading.buenchollotech.com`, que al generar el código MQL5 el pipeline fallaba con `Request failed: 502` (sin poder cambiar de modelo) y que al probar con TSLA el backtest automático no parecía usar el símbolo nuevo. Dos diagnósticos independientes con causa raíz confirmada en logs (`pm2`, log principal del terminal MT5, logs de los agentes del Strategy Tester), ambos desplegados a `main` (`8916be0`, `d5ad933`) con `pm2 restart` + health OK:

1. **Generación MQL5 frágil**: los jobs vivían solo en un `Map` en memoria y `pollJob` (frontend) abortaba ante cualquier error que no fuera un `TypeError` de red — un `pm2 restart` (redeploy, `update.ps1`) o un `502`/`504` transitorio del túnel de Cloudflare mataba la generación entera. Fix: jobs persistidos en `server/src/data/mql5-jobs/`, `reconcileOrphanedMql5Jobs` al arrancar (mismo patrón que `reconcileOrphanedRuns`), `pollJob` tolera `502/503/504` ~90 s, selector Fuente→Modelo→Esfuerzo también en el estado de error + recuperación desde el run, `MQL5_GEN_TIMEOUT_MS` 180→300 s.
2. **Backtest headless no corría con MT5 abierto**: `terminal64.exe /config:<ini>` solo lanza el tester en arranque limpio; contra una instancia ya abierta la config `[Tester]` reenviada se ignora en silencio. El par/timeframe SÍ se pasaban bien (el `.ini` de la prueba de TSLA en disco lo confirma). Fix: `isTerminalRunning` corta con aviso accionable en vez de esperar 180 s.

**Informe completo**: [`proyecto-dashboard/informes/2026-08-30-resiliencia-generacion-mql5-y-backtest-mt5-abierto.md`](proyecto-dashboard/informes/2026-08-30-resiliencia-generacion-mql5-y-backtest-mt5-abierto.md). Continúa los hallazgos 5 y 6 del informe del 2026-08-20. Actualizados `server/README.md` (jobs persistidos + guard de MT5 abierto), `docs/DESPLIEGUE_WEB_SIEMPRE_ACTIVA.md` (troubleshooting + bitácora) y `docs/MQL5_LECCIONES_LOGICA_Y_BACKTEST.md` (§5, nota operativa de MT5 abierto).

**Incoherencia detectada, no resuelta**: el `CLAUDE.md` raíz dice "este repositorio no usa `docs/`", pero `trading-agents-dashboard/docs/` tiene 5 ficheros activos y referenciados desde los README. Dejado como decisión pendiente para Pedro (ver §Pendiente del informe).
