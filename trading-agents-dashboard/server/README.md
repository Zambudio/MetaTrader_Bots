...
**Por qué `/generate` es un job asíncrono y no una respuesta directa:** una generación real tarda de 3 a 25 minutos (hasta 3 llamadas al LLM de hasta 300s cada una —`MQL5_GEN_TIMEOUT_MS`, ver `.env.example`—, más compilación, backtest headless de hasta 180s, hasta 3 ciclos de optimización del Quality Gate, y el bucle exterior `generateWithStrategyFeedbackLoop`). Devolver eso como una única respuesta HTTP bloqueante era frágil: **cualquier interrupción durante esos minutos — el `tsx watch` reiniciándose por una edición de código, un `pm2 restart` de `scripts/update.ps1`, o un `502` transitorio del túnel de Cloudflare en producción — tumba la conexión, y el navegador lo reporta como `TypeError: Failed to fetch` opaco**, aunque el trabajo en el servidor normalmente sí completase. Por eso `POST /generate` solo crea el job y responde al momento con `{ jobId }`; el cliente hace polling a `GET /generate/:jobId` cada 2s.

**Resiliencia del job (`mql5Jobs.ts`)**: el job vive en un `Map` en memoria (ruta rápida para el sondeo) **y además se refleja en disco** (`server/src/data/mql5-jobs/<id>.json`, ignorado por git). Al arrancar, `reconcileOrphanedMql5Jobs()` (llamado desde `index.ts` junto a `reconcileOrphanedRuns`, antes de `app.listen`) convierte todo job persistido en `running` — necesariamente huérfano en un proceso recién arrancado — en un estado terminal: `done` si su run ya tiene `mql5Result` (el pipeline terminó y guardó justo antes de que el proceso muriera), o `error` accionable en otro caso. Barre además ficheros caducados (TTL 30 min) y resiste ficheros corruptos sin tumbar el arranque. `GET /generate/:jobId` cae a disco (`getMql5JobPersisted`) si el job no está en memoria, así que tras un reinicio el frontend recibe un `error` claro en vez de un `404`. Por el lado del cliente, `pollJob` (`src/lib/api.ts`) tolera `502/503/504/429/500` y fallos de red durante ~90s seguidos antes de rendirse (antes abortaba ante cualquier error que no fuera un `TypeError` de red — un solo `502` del túnel mataba la generación entera), y si el sondeo falla consulta el run por si `mql5Result` ya está guardado. El `<ModelSelector>` (Fuente→Modelo→Esfuerzo) se muestra también en el estado de error, para cambiar de motor antes de reintentar.
...
Tras compilar limpio, `generateMql5`/`optimizeMql5` lanzan automáticamente un backtest real contra MetaTrader (`terminal64.exe /config:<ini>`, símbolo/timeframe/rango de la estrategia, 2025.08.01–2026.08.18), sin que el usuario pegue ningún log a mano. El `.ini` lleva `Symbol=normalizeSymbol(strategy.pair)` y `Period=strategy.timeframe` — el par y el timeframe del análisis **sí** se propagan al tester (verificado: un análisis de TSLA generó un `.ini` con `Symbol=TSLA`, `Period=H4`). `runHeadlessBacktest` sondea el log del agente de tester (`Tester/<perfil>/Agent-N/logs/`) hasta encontrar la marca `final balance` (backtest terminado) o agotar 180s, calcula el Quality Gate cuantitativo (Doc 20 §4: ≥15 operaciones, 0 rechazadas, esperanza >0.10R, PF≥1.20, beneficio neto>0, drawdown≤15%) sobre la sesión parseada, y si no lo pasa reintenta hasta 2 veces más regenerando el código con los motivos de fallo concretos (`buildOptimizationPrompt`) antes de entregar el EA.

**MetaTrader 5 debe estar cerrado** al generar. `terminal64.exe /config:<ini>` solo ejecuta el tester en un **arranque limpio**; si ya hay una instancia de `terminal64.exe` abierta (p. ej. mirando gráficos), Windows le reenvía la config por línea de comandos y el proceso nuevo sale al instante — pero la instancia en marcha **ignora la sección `[Tester]` reenviada** y el backtest nunca corre (confirmado en los logs del terminal: una prueba con MT5 abierto no deja ni una línea de `Startup`/`Tester`). `isTerminalRunning()` comprueba con `tasklist` antes de lanzar; si MT5 está abierto, `runHeadlessBacktest` corta de inmediato con un aviso accionable (`qualityNotes`) en vez de esperar 180s a un timeout mudo, y el EA compilado se entrega igualmente (`retriableCodeIssue` queda `undefined` — no es un problema de código, así que no dispara el bucle de auto-optimización). Aislar el backtest de la instancia interactiva (segunda instalación de MT5 o `--portable`) queda pendiente — ver [`wiki-Traiding/proyecto-dashboard/informes/2026-08-30-resiliencia-generacion-mql5-y-backtest-mt5-abierto.md`](../../wiki-Traiding/proyecto-dashboard/informes/2026-08-30-resiliencia-generacion-mql5-y-backtest-mt5-abierto.md).
