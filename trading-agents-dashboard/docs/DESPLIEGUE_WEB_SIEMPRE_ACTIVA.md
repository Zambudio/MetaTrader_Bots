...
cloudflared.exe   ← proceso pm2 "trading-tunnel"  (C:\Users\fadwe\cloudflared\)
   │  http://localhost:5175
   ▼
Express (server/, vía tsx)   ← proceso pm2 "trading-dashboard"
...
| `server/package.json` | Script `start` = `tsx --env-file-if-exists=.env src/index.ts` (producción, sin `watch`). |
| `package.json` (raíz) | Script `start` = `npm run start --prefix server`. |
| `ecosystem.config.cjs` | **Nuevo.** Define las 2 apps pm2 (`trading-dashboard`, `trading-tunnel`). |
...
> aceptan UNC; por eso pm2 lanza `tsx` directamente, no `npm run`.
...
**Pendiente de probar por el usuario:** un reinicio real del PC + login, y una
ejecución completa de análisis + generación MQL5 desde la URL pública.
...
git checkout main -- server/src/index.ts server/package.json package.json
```
